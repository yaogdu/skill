...
(function($) {
	...
	var DataTable = function(element, options) {
		...
		this.localStorageId = "datatable_"
				+ (options.id || options.url.replace(/\W/ig, '_'));
		...
		this.$default = this.$element.children().length ? this.$element
				.children() : $("<div></div>").addClass("alert alert-error")
				.html("无数据返回");
		...
		if (this.options.tablePreRender
	...
	DataTable.prototype = {
		...
		render : function() {
			...
			// bottom
			this.$bottom_details = $("<div></div>").attr("id",
					"dt-bottom-details");
			...
			if (o.url !== "") {
				$.ajax({
					...
					success : function(res) {
						...
						// set the current page if we're forcing it from the
						// server
						if (res.currentPage)
							o.currentPage = parseInt(res.currentPage);

						// retrieve the saved columns
						_retrieveColumns.call(that,
								localStorage[that.localStorageId])
						...
						if (o.showTopPagination && that.pagination())
							that.$top_details.append(that.pagination().clone(
									true));
						if (o.showPagination && that.pagination())
							that.$bottom_details.append(that.pagination()
									.clone(true));
						...
						// nearly complete... let the user apply any final
						// adjustments
						if (o.tableCallback
								&& typeof o.tableCallback === 'function')
		...
		,
		toolbar : function() {
			var o = this.options, $e = this.$element, that = this;

			this.$toolbar = $("<div></div>").addClass(
					"dt-toolbar btn-toolbar pull-right");

			this.$button_group = $("<div></div>").addClass("btn-group")
			...
			if (o.sectionHeader) {
				...
				this.$section_header = $("<h4></h4>").text(o.title).append(
						this.$toolbar);
		...
		,
		details : function() {
			var o = this.options, res = this.resultset, start = 0, end = 0, that = this;
			...
			$(
					'<div class="pull-left"><p>显示' + start + '至' + end + '条，总计'
							+ o.totalRows + '条记录</p></div>').prependTo(
					this.$bottom_details);
		}
		...
		,
		header : function() {
			var o = this.options, res = this.resultset;
		...
		footer : function() {
			...
			if (!this.$footer) {
				...
				for (column in o.columns) {
					...
					$cell.data("cell_properties", o.columns[column]).addClass(
							o.columns[column].classname)
		...
		,
		body : function() {
			var res = this.resultset, o = this.options;
		...
		,
		filter : function() {
			var $row = $("<tr></tr>"), o = this.options, that = this;
			...
			for ( var column in o.columns) {
				...
				if (o.columns[column].hidden)
					$cell.hide();

				if (o.columns[column].filter && o.columns[column].field) {
					$cell.append($("<input/>").attr("name",
							"filter_" + o.columns[column].field).data("filter",
							o.columns[column].field).val(
							o.filter[o.columns[column].field] || "")
		...
		cell : function(data, column) {
			var celldata = data[this.options.columns[column].field]
					|| this.options.columns[column].custom, $cell = $('<td></td>'), o = this.options;

			// preprocess on the cell data for a column
			if (o.columns[column].callback
					&& typeof o.columns[column].callback === "function")
				celldata = o.columns[column].callback.call($cell, data,
						o.columns[column])

			$cell.data("cell_properties", o.columns[column]).addClass(
					o.columns[column].classname).append(celldata || "&nbsp;")

			if (o.columns[column].css)
				$cell.css(o.columns[column].css);
		...
		column : function(column) {
			var $cell = $('<th></th>'), o = this.options, classname = "dt-column_"
					+ column + Math.floor((Math.random() * 1000) + 1);

			o.columns[column].classname = classname;

			$cell.data("column_properties", o.columns[column]).addClass(
					classname).text(o.columns[column].title);
		...
		,
		sort : function(e) {
			var colprop = $(this).data("column_properties"), that = e.data, o = e.data.options, found = false;

			colprop.sortOrder = colprop.sortOrder ? (colprop.sortOrder == "asc" ? "desc"
					: "")
					: "asc";
		...
		,
		pagination : function() {
			var $e = this.$element, that = this, o = this.options, res = this.resultset;

			// no paging needed
			if (o.perPage >= res.totalRows)
				return;

			if (!this.$pagination) {
				this.$pagination = $("<div></div>").addClass(
				...
				var $pager = $("<ul></ul>"), $first = $("<li></li>").append(
						$("<a></a>").attr("href", "#").data("page", 1).html(
								"最前页").click(function() {
							o.currentPage = 1
							that.render();
							return false;
						...
						.append(
								$("<a></a>")
										...
										.click(
												function() {
													o.currentPage -= 1
													o.currentPage = o.currentPage >= 1 ? o.currentPage
															: 1
													that.render();
													return false;
												})), $next = $("<li></li>")
						.append(
								$("<a></a>")
										...
										.click(
												function() {
													o.currentPage += 1
													o.currentPage = o.currentPage <= o.pageCount ? o.currentPage
															: o.pageCount
													that.render();
													return false;
												})), $last = $("<li></li>")
						.append(
								$("<a></a>").attr("href", "#").data("page",
										o.pageCount).html("最后页").click(
										function() {
				...
				var totalPages = o.pagePadding * 2, start, end;
	...
	function _initColumnModal() {
		var o = this.options, $e = this.$element, $top_details = this.$top_details, $toggle = $("<a></a>");

		// localize the object
		var that = this;

		if (!this.$column_modal) {
			this.$column_modal = $('<div></div>').attr("id",
					"dt-column-modal_" + Math.floor((Math.random() * 100) + 1))
					.addClass("modal").hide();

			// render the modal header
			this.$column_modalheader = $("<div></div>")
					.addClass("modal-header").append(
							$("<button></button>").addClass("close").data(
									"dismiss", "modal").text('x').click(
									function() {
										that.$column_modal.modal('hide');
									})).append(
							$("<h3></h3>").text("Toggle Columns"));
			...
			this.$column_modalfooter = $("<div></div>")
					...
					.append(
							...
							,
							o.allowSaveColumns ? $("<a></a>").attr("href", "#")
									.addClass("btn btn-primary").text("Save")
									.click(function() {
			...
			this.$column_modal.append(this.$column_modalheader,
					this.$column_modalbody, this.$column_modalfooter).appendTo(
					document.body);
		}

		// render the display modal button
		$toggle.addClass("btn").data("toggle", "modal").attr("title",
				"Choose which columns you would like to display.").attr("href",
				"#" + this.$column_modal.attr("id")).html(
				"<i class=\"icon-cog\"></i>").click(function(e) {
			that.$column_modal.on('show', function() {
	...
	function _initFilterModal() {
		var o = this.options, $e = this.$element, $toggle = $("<a></a>");

		// render the display modal button
		$toggle.addClass("btn").data("toggle", "modal").attr("href", "#").attr(
				"title", "Open the filter dialog.").html(
				"<i class=\"icon-filter\"></i>").click(function() {
	...
	function _initPerPage() {
		...
		var $perpage_select = $("<a></a>").addClass("btn dropdown-toggle")
				.attr("title", "Change the number of rows per page.").attr(
						"data-toggle", "dropdown").html(o.perPage + "&nbsp;")
				.css({
					fontWeight : 'normal'
				}).append($("<span></span>").addClass("caret"));
		this.buttons.push($perpage_select);

		var $perpage_values = $("<ul></ul>").addClass("dropdown-menu").css({
			fontSize : 'initial',
			fontWeight : 'normal'
		}).append(
				$('<li data-value="5"><a href="#">5</a></li>').click(
						function() {
							_updatePerPage.call(this, that);
							return false;
						}),
				$('<li data-value="10"><a href="#">10</a></li>').click(
						function() {
							_updatePerPage.call(this, that);
							return false;
						}),
				$('<li data-value="20"><a href="#">20</a></li>').click(
						function() {
							_updatePerPage.call(this, that);
							return false;
						}),
				$('<li data-value="50"><a href="#">50</a></li>').click(
						function() {
							_updatePerPage.call(this, that);
							return false;
						}),
				$('<li data-value="100"><a href="#">100</a></li>').click(
						function() {
							_updatePerPage.call(this, that);
							return false;
						}));
	...
	function _initTableInfo() {
		var o = this.options, $e = this.$element, $info = $("<a></a>");

		// render the display modal button
		$info.addClass("btn").attr("href", "#").html(
				"<i class=\"icon-info-sign\"></i>").click(function() {
			return false;
		});

		var $page_sort = [], $page_filter = [];
		...
		$.each(o.sort, function(i, v) {
			...
			var heading;
			for ( var column in o.columns) {
				if (o.columns[column].field == v[0])
					heading = o.columns[column].title;
			}
				...
				.popover(
						{
							...
							content : $('<dl></dl>')
									.append(
											$page_sort.length > 0 ? '<dt><i class="icon-th-list"></i> Sort:</dt><dd>'
													+ $page_sort.join(", ")
													+ '</dd>'
													: '',
											$page_filter.length > 0 ? '<dt><i class="icon-filter"></i> Filter:</dt><dd>'
													+ $page_filter.join(", ")
													+ '</dd>'
													: '')
						});
	...
	function _initOverflowToggle() {
		var o = this.options, $wrapper = this.$wrapper, $overflow = $("<a></a>");
		...
		$overflow
				...
				.html("<i class=\"icon-resize-full\"></i>").click(function() {
					if ($wrapper)
	...
	function _updateColumnModalBody(body) {
		var o = this.options, $container = $("<fieldset></fieldset>"), that = this;

		// loop through the columns
		for ( var column in o.columns) {
			if (o.columns[column].title === "")
				continue;
			var $item = $(
					'<div class="control-group" style="float: left" data-column="'
							+ column
							+ '"><label class="control-label">'
							+ o.columns[column].title
							+ '</label><div class="controls"><div class="btn-group" data-toggle="buttons-radio"><a href="#" class="btn '
							+ (o.columns[column].hidden ? "" : "active")
							+ '"><i class="icon-ok"></i></a><a href="#" class="btn '
							+ (o.columns[column].hidden ? "active" : "")
		...
		body.empty();
		body.append($("<form></form>").addClass("form-horizontal").append(
				$container));
	}

	function _toggleColumn(that) {
		var o = that.options, column = $(this).data("column"), $column = $("."
				+ o.columns[column].classname);
		...
		o.columns[column].hidden ? $(this).find(".icon-remove").parent()
				.addClass("active") : $(this).find(".icon-ok").parent()
				.addClass("active");
	...
	function _retrieveColumns(data) {
		var o = this.options, columns = data ? JSON.parse(data) : [], res = this.resultset;
		...
		for (column in o.columns) {
			o.columns[column] = $.extend({}, o.columns[column],
					res.columns[column], columns[column]);
		}
	}
	...
	$.fn.datatable.init = function(options, Constructor, name) {
		...
		function get(el) {
			...
			if (!datatable) {
				datatable = new Constructor(el, $.fn.datatable.elementOptions(
						el, options));
